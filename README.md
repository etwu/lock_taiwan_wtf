...
- **嘟文模式**：自動檢查加密後長度是否在發文上限（1000 字）內，並附上 `#encrypted_toots` 標籤，方便未來的自動解密工具辨識。
